...
func initialize(stat_manager: StatManager, card_information_manager: CardInformationManager):
	cardInformationManager = card_information_manager
	statManager = stat_manager

func next_card():
	var newInformation = cardInformationManager.next_card({"stats": statManager.stats, "tags": currentTags})
	set_current_card()
